...
var sourceSqliteConnectionString =
    Environment.GetEnvironmentVariable("Tubester_MIGRATOR_SOURCE")
...
var destinationPostgresConnectionString =
    Environment.GetEnvironmentVariable("Tubester_MIGRATOR_DEST")
...
var sourceDbContextOptions = new DbContextOptionsBuilder<TubesterDb>()
    .UseSqlite(sourceSqliteConnectionString)
    .Options;

var destinationDbContextOptions = new DbContextOptionsBuilder<TubesterDb>()
    .UseNpgsql(destinationPostgresConnectionString)
    .Options;

await using var sourceDbContext = new TubesterDb(sourceDbContextOptions);
await using var destinationDbContext = new TubesterDb(destinationDbContextOptions);
...
// FK-safe order for current TubesterDb model:
...
static async Task CopyTableAsync<TEntity>(
    ...
    TubesterDb destinationDbContext,
